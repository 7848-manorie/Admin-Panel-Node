...
module.exports.checkChangePassword = async (req, res) => {
    try {
        ...
        const isMatch = await bcrypt.compare(req.body.currentPass, oldPass);

        if (isMatch) {
            if (oldPass !== req.body.npassword) {
                if (req.body.npassword == req.body.cpassword) {
                    const hashedNewPass = await bcrypt.hash(req.body.npassword, 10);
    ...
    try {
        ...
        if (checkEmailId) {
            const transporter = nodemailer.createTransport({
                ...
                secure: false,
                auth: {
                    user: "7848manoriechauhan@gmail.com",
                    pass: "ykrhptwkntjxcoir",
                },
            });
            ...
            const info = await transporter.sendMail({
                from: '<7848manoriechauhan@gmail.com>',
                to: req.body.email,
...
module.exports.forgotPassword = async (req, res) => {
    try {
        ...
        if (req.body.npass == req.body.cpass) {
            ...
            if (checkEmail) {
                const hashedPass = await bcrypt.hash(req.body.npass, 10);
...
module.exports.editAdminData = async (req, res) => {
    try {
        ...
        let updateData = {
            ...
            email: req.body.email,
            password: oldAdminData.password,
